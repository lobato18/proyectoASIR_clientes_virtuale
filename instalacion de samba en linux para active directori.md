...
DOMAIN="tecnolobato.local"
REALM="TECNOLOBATO.LOCAL"
NETBIOS_NAME="TECNOLOBATO"
ADMIN_PASS="Password123!" # ¡Cambia esta contraseña por una segura!
...
DEBIAN_FRONTEND=noninteractive apt install -y \
    samba krb5-config krb5-user winbind libpam-winbind \
    libnss-winbind smbclient python3-samba /usr/bin/expect

echo "--- 2. Limpiando configuración previa ---"
# Detener servicios estándar de Samba
systemctl stop smbd nmbd winbind
systemctl disable smbd nmbd winbind
systemctl unmask samba-ad-dc

# Mover el archivo original de samba para empezar de cero
if [ -f /etc/samba/smb.conf ]; then
    mv /etc/samba/smb.conf /etc/samba/smb.conf.bak
fi

echo "--- 3. Aprovisionando el Dominio Active Directory ---"
# Se usa --use-rfc2307 para habilitar extensiones de Unix (UID/GID)
...
echo "--- 4. Configurando Kerberos ---"
# Samba genera un krb5.conf que debemos usar
if [ -f /var/lib/samba/private/krb5.conf ]; then
    cp /var/lib/samba/private/krb5.conf /etc/krb5.conf
fi

echo "--- 5. Iniciando el servicio AD DC ---"
systemctl enable samba-ad-dc
systemctl start samba-ad-dc
...
echo "--------------------------------------------------------"
echo "Recuerda apuntar el DNS de este servidor a su propia IP."
